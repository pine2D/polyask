...
- 档位：think = `_selectModel(/fable\s*5/i)` + `_setEffort("top")`（**取在场最高档**，当前是 Max，复读 `_THINK`）；fast = `_selectModel(/sonnet\s*5/i)` + `_setEffort("bottom")`（**取在场最低档**，当前是 Low，复读点中那一项的首词）。**effort 是站点级记忆，换模型不重置**：1.0.1 之前 fast 只换模型不压档，think 过一次后快档会以「Sonnet 5 · Max」发出去（用户真机 2026-09-16）。`state()`：sonnet/haiku 带 `_THINK` 命中的高档 effort 判 null（不是预设档），否则 fast；Fable/Opus 按 effort 后缀判。
