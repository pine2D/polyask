...
| Kimi | 已稳定会话的 L 轮正确保存 22 字符合成标记与地址。部分首屏轮次临时消息回退/回到旧测试会话；K 轮实际显示旧 D 消息，归属规则拒绝将其保存为 K。该首屏路径仍待专项复现。 |
...
仍未验收：九站全部重新生成/编辑重发/回答分支控件（尤其没有 button/a/role 的自定义控件）、长思考/全部档位、图片、多账号登录重定向、真实 Drive 双设备与旧客户端、Windows/macOS 原生。没有已授权且连接的两台 Drive 测试设备，本轮只执行本地同步/备份生产模块测试；不把它记作真实云端验收。不进行自动重发，不开启 Kimi 自动重发开关。
