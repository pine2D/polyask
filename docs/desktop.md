...
- 品牌沿用靛蓝：亮色 `#4f46e5`、暗色 `#a5a0ff`，成功 `#16a34a`、失败 `#dc2626`，其余用系统中性色；字体 `system-ui`，不捆绑字体。站点标题条按选择标签、状态圆点与文字、常驻操作排列；加载时底边显示不定进度细线，由真实 loading 阶段驱动，就绪或失败即停止，不显示估算百分比，减少动态效果时显示静态细线。不加装饰渐变与无信息动画。布局切换不为原生视图 bounds 伪造动画。
- 阅读空间优先，命令栏保持单行：发送按钮显示目标数，窄窗保留图标与数字，完整操作名由三语 `aria-label`/提示提供；采集并比较和结果库始终使用等宽图标按钮。结果与对照正文用 14px / 1.75 行距，代码块用 13px / 1.6，不改变原站网页字号。
