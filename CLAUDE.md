...
- **Claude 档位映射**：think=Opus 4.8 + Thinking 开 + effort Max；fast=**Sonnet 5 默认设置**（Sonnet 5 不提供 effort 选择器，勿在选模后继续操作 Thinking/effort 菜单；state() 见模型名带 sonnet 恒判 fast）。Anthropic 换代时同步 `fast()`/`think()` 里的模型正则。
