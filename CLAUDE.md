...
- 适配器契约：每站 `{think, fast, state(), diagnose()}`（可选 `submit(el)`、`answer()`）；state/diagnose/answer **只读不开菜单**。
