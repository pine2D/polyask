...
- storage keys：`amsConsole{selected,tier,prompt}`、`amsWindows`、`amsHistory`(≤20 去重)、`amsTemplates`、`amsGroups`、`amsConsoleWin`、`amsComposeWin`、`amsAutoRaise`(bool，popup 开关：发送后自动置顶)、`amsSessions`(host→会话 URL)+`amsRestorePending`(onStartup 置位、首次 openTile 消费)——会话恢复**仅限重启后首次开窗**，平时新窗一律空白新会话（曾因每窗都续旧对话被用户当 bug 报）；「全部新会话」清 amsSessions。注意窗口 id 类 key 在 `onStartup` 一律清空（id 跨重启失效），`amsSessions` 有意不清。
