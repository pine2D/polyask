...
**单向同步**：本仓是**唯一事实源**，只在本仓改适配器，改完跑一次脚本推到姊妹仓（不再双向、不再手改 sch）：
```bash
scripts/sync-to-sch.sh   # content/adapters-*.js → sch custom/，自动替换前缀(__AMS→__SCH、content/→custom/)
```
姊妹仓的适配器视为生成物，勿直接改它（改了会被下次 push 覆盖）。
