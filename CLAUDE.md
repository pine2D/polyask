...
<!-- 最后与代码核对：2026-09-20 · desktop/package.json v1.0.2（含未发布改动）。发版前重跑核对并更新这行。
...
- **`diagnose()` 每条检查必须带 `kind`**（`reach`/`control`/`tier`/`probe`）。只让**非 `tier`** 的红项决定站点可用性——各站 `state()` 是刻意的偏函数，用户停在非预设的合法档位（千问 Qwen3.7+快速、Kimi Instant、元宝 Thinking）都返回 null，那不是故障。漏标会被归成 `control` 继续误报，`desktop/scripts/diag-runtime.test.js` 守着。
...
- **数据与同步**：`main/database.ts`（SQLite）+ `*-repository.ts`、`sync-engine.ts` / `sync-pull.ts` / `drive-client.ts`（Drive appdata，旧实体 schema 1 / 决策卡 schema 2 / 文件夹及关联 schema 3，fixture 在 `desktop/test/fixtures/`）、`data-admin-service.ts`（本机数据管理）、`backup-service.ts`（预览与事务恢复）。
