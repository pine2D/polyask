...
- storage keys：`amsConsole{selected,tier,prompt}`、`amsWindows`、`amsHistory`(≤20 去重)、`amsTemplates`、`amsGroups`、`amsConsoleWin`、`amsComposeWin`、`amsSessions`(host→会话 URL，跨重启续上；「全部新会话」清除)。注意窗口 id 类 key 在 `onStartup` 一律清空（id 跨重启失效），`amsSessions` 有意不清。
