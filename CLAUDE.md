...
加站点改两处（漏一处=该站静默不工作）：`manifest.json` 的 `matches` + 对应适配器（intl 或 cn/cn2）。群发的发送/切档复用 `content/core.js` 的通用 `submitPrompt`/`runMode`，多数站点无需在适配器写 `submit`；仅当该站发送键不带 `send/发送` 标签、且 Enter 提交不灵时，才在适配器加可选 `submit(el)`（原生点其真实发送键）。`console/console.js` 的 `SITES` 列表与站点预设 `PRESETS` 也要同步加。
...
- storage keys：`amsConsole{selected,tier,prompt}`、`amsWindows`、`amsHistory`(≤20 去重)、`amsTemplates`、`amsGroups`、`amsConsoleWin`、`amsComposeWin`、`amsSessions`(host→会话 URL)+`amsRestorePending`(onStartup 置位、首次 openTile 消费)——会话恢复**仅限重启后首次开窗**，平时新窗一律空白新会话（曾因每窗都续旧对话被用户当 bug 报）；「全部新会话」清 amsSessions。注意窗口 id 类 key 在 `onStartup` 一律清空（id 跨重启失效），`amsSessions` 有意不清。
...
- **adapter.submit 契约**：返回 `false`=发送键此刻不可用 → 落回通用路径；点击成功也必须过 `confirmSubmitted`（输入框清空/变化=已发出）再判成功。已知限制（真机证实）：聊天站流式生成期间普遍把发送键复用为「停止」（class/id 不变仅换图标），流式中对同站二次群发会点成停止、截断上一条——诚实报失败可 retry，图标判别脆弱故不做守卫。
...
- **Claude 档位映射**：think=Opus 4.8 + effort Max；fast=**Sonnet 5** + 思考开 + effort Medium（`_setThinking(on, effort)` 参数化 effort，点 `effort-option-<level>`；state() 见模型名带 sonnet 恒判 fast）。Anthropic 换代时同步 `fast()`/`think()` 里的模型正则。
...
- 发版三步缺一不可：manifest version → **CHANGELOG.md 写本版分类段落**（release.yml 以其为 Release 正文、缺段落拒发，不用自动 compare 链接）→ tag v* 推送。
