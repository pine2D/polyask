...
在结果详情点「创建决策卡」，记录采用结论、理由、待核实事项与下一步。每条结果可创建多张卡，每张卡能关联其中多份回答的逐字摘录；在结果库的「决策卡」视图中搜索、按状态筛选和导出 Markdown。状态由你手动维护，“已定稿”不代表已完成事实核实。删除来源结果会保留决策卡与此前保存的摘录，并标注来源已删除；要删除卡片与摘录，请删除决策卡或在设置中单独清空。
...
多台电脑同步。连上 Google Drive 之后，站点选择、分组、提问历史、提示词模板、结果库和决策卡会在你的设备之间合并。PolyAsk 只用 Drive 里自己的隐藏目录，碰不到你的其他文件。同步决策卡后，旧版客户端检测到新格式会停止同步写入，需升级才能恢复；原有历史和回答快照仍可读取。
